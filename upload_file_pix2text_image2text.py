...
from api.utils_scanssd.crop_resize_image import crop_pix2text
...
path_list = ['images','pdf','results','annotation','crop']
...
def predict(
    ...
    for idx, fp in enumerate(fp_list):
        ...
        ## Realizando o Crop da imagem para utilizar na biblioteca 
        for o in out:
            if o['type'] == 'isolated':
                i = i+1
                crop_pix2text(fp,[o["position"][3]+o["position"][1]],os.path.join(root_folder,uuid_folder,'crop'),i)
                
        #res = res.join('\n'.join([o['text'] for o in out if o['type']=='isolated']))

    #create_latex_file(root_folder,uuid_folder,res)
    #Gera o latex a partir do crop das imagens
    image2latex(os.path.join(root_folder,uuid_folder,'crop'),os.path.join(root_folder,uuid_folder))
